...
public final class IntakeResultPage extends BasePage {
	public IntakeResultPage(WebDriver driver) {
	    ...
	    By successAlert = By.cssSelector(".alert.alert-success, .toast-success, .swal2-success");
	    By resultHeader = By.xpath("//h5[contains(normalize-space(),'Thông tin lượt khám') or contains(normalize-space(),'lượt khám')]");
	    By queueBadge   = By.xpath("//*[contains(normalize-space(),'Số thứ tự') or contains(@class,'queue')]");

	    wait.until(d ->
	        d.findElements(successAlert).size() > 0
	        || d.findElements(resultHeader).size() > 0
	        || d.findElements(queueBadge).size() > 0
	        || d.getCurrentUrl().toLowerCase().contains("intake")
	    );
	}
